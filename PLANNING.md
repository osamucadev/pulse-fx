...
## Decisão: Yarn Classic em todo o projeto (frontend e backend)

Yarn Classic (v1.22.22) é o gerenciador de pacotes usado no projeto inteiro, tanto no `frontend/` quanto no `backend/`. Cada pasta mantém seu próprio lockfile (`frontend/yarn.lock`, `backend/yarn.lock`), sem workspace compartilhado entre elas (ver seção de estrutura do repositório, mais acima); a padronização é só na ferramenta usada pra instalar dependências, não um monorepo com pacotes compartilhados.

O motivo original, que levou o frontend a adotar Yarn primeiro, é técnico, não uma preferência arbitrária. A versão do Vite usada no scaffold (8.x) usa o Rolldown como motor de bundling, que declara binários nativos para várias plataformas como `optionalDependencies` no `package.json`. Isso faz o processo de instalação disparar um número grande de requisições pequenas ao registry, uma por combinação de plataforma/arquitetura. Com `npm create vite@latest`, essa instalação travava indefinidamente, sem erro, sem timeout, só nunca terminava. `curl` e `npm ping` confirmaram que a rede e o registry estavam saudáveis, então o problema era específico de como o cliente do npm lida com esse volume de `optionalDependencies` do Rolldown, não da rede em si. Trocando para Yarn Classic, com `yarn create vite . --template react-ts`, a mesma instalação completou em segundos, sem travar.

Nessa época, o backend continuou com npm por pragmatismo, sem motivo técnico específico, só porque já estava funcionando e não valia parar pra trocar no meio do desenvolvimento inicial. Isso deixou o projeto com dois gerenciadores de pacotes diferentes, uma inconsistência registrada como dívida técnica conhecida (documentada na página "Sobre" do frontend). Depois, já com o backend estável e com ESLint configurado (ver decisão acima), essa dívida foi resolvida: o `backend/package-lock.json` foi removido, um `backend/yarn.lock` foi gerado a partir do `package.json` existente, e o `Dockerfile` do backend passou a usar `yarn install --frozen-lockfile` (equivalente ao `npm ci`) e os comandos `yarn build`/`yarn start` no lugar dos equivalentes em npm. Por isso, todo comando de instalação ou script em `frontend/` ou `backend/` deve usar `yarn`, nunca `npm`.
